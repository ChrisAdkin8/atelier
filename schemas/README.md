...
| `config/hook_manifest.v1.json` | Hook manifest — registers a pre-tool / post-tool / on-verify-* hook (event, optional tool_filter, shell implementation, time_budget_ms). HTTP hook manifests are reserved until a production HTTP executor exists; the shell shape is intentionally inlined (not shared) to keep tool-only `timeout_ms` out. Examples in `examples/hooks/`. | §15 |
...
| `config/dod.v1.json` | Per-repo definition-of-done — list of `(name, tier, command, expect)` checks the Verifying state runs after `claimed_done: true`. Loaded from `<repo>/.atelier/dod.json` with optional `~/.atelier/dod.json` fallback. Example: `examples/config/dod.v1.json`. Distinct from `workload/task_checks.v1.json` (which is per-fixture in the rig). | §7 |
