...
| `src/config.rs` | §1 / §11 (v53) | `ProvidersConfig` TOML loader for `.atelier/providers.toml`. Multi-profile shape: `default = "<name>"` + `[providers.<name>]` map + optional top-level `[runner]` / `[probe]`. `resolve_profile(cli)` picks CLI > `default` > none; `validate()` enforces `default` references an existing profile and `base_url` requires `openai-compat`. `serde(deny_unknown_fields)` everywhere so typos are visible. 19 unit tests cover shape, discovery, all three resolution paths, cross-section validation, and round-trip. |
