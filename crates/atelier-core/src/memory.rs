...
pub struct MemoryCard {
    ...
    pub pinned: bool,
    /// v60.5 — set when this card was generated by §5 non-destructive
    /// compaction. Points at the on-disk blob the v60.6 Expand affordance
    /// reads to restore the originals. `None` for every other card so
    /// existing sessions round-trip unchanged.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub compacted_from: Option<CompactionSource>,
}

/// v60.5 — provenance link from a summary [`MemoryCard`] back to the
/// context items it replaced + the on-disk blob carrying their full
/// contents. Set exclusively by `SessionDispatcher::compact_context_items`;
/// read by the v60.6 Expand path (and rendered as a "compacted from N
/// items" badge in the §5 Memory pane).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct CompactionSource {
    /// Context-item ids (as stored in the §5 `ContextManager`) that were
    /// evicted in this compaction.
    pub item_ids: Vec<String>,
    /// Repo-relative path under `.atelier/sessions/<sid>/compactions/` of
    /// the JSON blob carrying the original `ContextItem`s. The path shape
    /// is fixed by `atelier_cli::compaction_blob` so v60.6 can resolve it
    /// against the workspace root without parsing.
    pub expansion_blob_path: String,
    /// RFC 3339 — when the compaction ran. Distinct from
...
pub struct MemoryCardSummary {
    ...
    pub pinned: bool,
    /// v60.5 — `Some(N)` when this card was produced by a §5
    /// compaction; `N` is the number of items it replaced. Lets the
    /// Memory pane render a "compacted from N items" badge (and, in
    /// v60.6, an Expand button) without round-tripping the full
    /// [`CompactionSource`] over the bus. Absent on the wire and in
...
mod tests {
    ...
    // ---------- v60.5: compacted_from provenance link ----------

    fn compaction_source_fixture() -> CompactionSource {
        CompactionSource {
            item_ids: vec!["ctx-a".into(), "ctx-b".into()],
            expansion_blob_path:
    ...
    #[test]
    fn memory_card_serde_with_compacted_from() {
        let mut c = fixture_card("mem-1", "summary line\nbody");
        c.pinned = true;
        c.compacted_from = Some(compaction_source_fixture());
        let json = serde_json::to_value(&c).unwrap();
        assert_eq!(json["compacted_from"]["item_ids"][0], "ctx-a");
        assert_eq!(json["compacted_from"]["item_ids"][1], "ctx-b");
        let back: MemoryCard = serde_json::from_value(json).unwrap();
        assert_eq!(back, c);
    }

    #[test]
    fn memory_card_serde_without_compacted_from_is_backwards_compat() {
        // The pre-v60.5 bundled fixtures don't carry `compacted_from`.
        let raw = r#"{
            "id": "mem-1",
            "content": "alpha",
            "created_at": "2026-05-17T10:00:00Z",
            "last_used": "2026-05-17T10:00:00Z"
        }"#;
        let card: MemoryCard = serde_json::from_str(raw).unwrap();
        assert!(card.compacted_from.is_none());
        // And serialising back must NOT add the field.
        let json = serde_json::to_value(&card).unwrap();
        assert!(json.get("compacted_from").is_none());
    }

    #[test]
    fn compaction_source_round_trips_through_serde() {
    ...
    #[test]
    fn memory_card_summary_projects_compacted_from_count() {
        let mut c = fixture_card("mem-1", "title\nbody");
        c.compacted_from = Some(compaction_source_fixture());
        let s = MemoryCardSummary::from_card(&c);
        assert_eq!(s.compacted_from, Some(2));

        // And without it the summary stays absent (omitted on the wire).
