...
### svelte-check chokes on HTML-tag-like tokens inside `<script>` comments (real compiler doesn't)

**Failure**: adding a per-letter wordmark animation to `Header.svelte`, a JS comment in `<script>` read `// ...animation in <style>).`. `npm run build` (vite-plugin-svelte, the real Svelte compiler — treats `<script>` content as opaque raw text) compiled fine, but `npm run check` (svelte-check → svelte2tsx) tokenized the stray `<style>` tag, threw off tag balancing, and reported a misleading ``ERROR `<script>` was left open`` at the final `</style>` line (EOF) plus "Header.svelte has no default export" in the importer. Cost ~5 iterations because the error location pointed at the wrong line and `build` masked it.

**Prevention**: never put angle-bracketed tag-like text (`<style>`, `<script>`, `<div>`, etc.) or Svelte logic-block tokens (`{#each}`, `{/each}`, `{#if}`) inside comments anywhere in a `.svelte` file — not in `<script>` JS comments, not in markup `<!-- -->` comments. svelte2tsx scans them even where the runtime compiler ignores them. When `svelte-check` reports `element_unclosed` / "left open" at EOF with no obvious unclosed tag, suspect a tag/mustache-like token in a comment, and trust `svelte-check` over `npm run build` (vite caches transforms and treats script bodies as opaque, so it won't catch this). Phrase such comments in prose: "the style block", "the each loop".

---

## 2026-06-04 — Provider unreachability + GUI layout failure modes

### An unreachable OpenAI-compat endpoint silently shows 8K context window rather than erroring

**Failure**: the GUI footer showed `context 0 / 8,192` after startup against the vLLM ELB. The 8,192 is `DEFAULT_CONTEXT_WINDOW_TOKENS` — the conservative floor baked into `OpenAiCompatAdapter`. The probe (`GET /v1/models`) is supposed to overwrite it with vLLM's real `max_model_len` (32,768 for this model), but the ELB's SG rule had drifted to an old IP and the probe connection timed out silently. The adapter kept the fallback without any visible indication. The badge tooltip's `source:` field (`adapter` vs `probe`) is the only diagnostic, and it requires hovering.

**Prevention**: (1) the connect timeout fix (v60.98) means a dead endpoint fails in 10s rather than 600s, so the probe no longer quietly stalls for ten minutes — but the fallback value is still used and still looks "correct." (2) When a context-window figure looks suspiciously round (8K, 4K, 2K), check the tooltip `source:` line: `adapter` means the probe didn't update the seed (fallback in use), `probe` means the real value was read. (3) The real fix for an unreachable provider is infrastructure, not adapter config — check the SG rule (`aws ec2 describe-security-groups`) against your current IP (`curl checkip.amazonaws.com`) before debugging the Rust code.

### CSS grid `minmax(0, 1fr)` side tracks don't prevent overflow from flex children with intrinsic widths
