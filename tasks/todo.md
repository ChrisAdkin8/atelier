...
- [~] Built-in tools exposed via internal MCP-compatible interface (file ops, shell, grep, ast-grep) — v60.11 (C2). Partial: **external MCP-server tools** now register alongside built-ins via `McpToolWrapper` (impls `Tool` trait by routing to `McpServerHandle::call_tool`) + `register_mcp_servers(registry, manifests, approvals, sandbox, audit_dir)` helper returning `RegisterMcpReport { servers_registered, tools_registered, servers_pending_approval, server_failures }`. Still open: refactor the **built-ins themselves** (read_file, write_file, edit_file, list_dir, grep, ast_grep, shell, spawn_subagent) to register via the same `McpToolWrapper`-style surface so the dispatcher loop is uniform. Deserves a focused single-session pass — touches every existing tool impl.
