...
- [~] Capability matrix as machine-readable config + UI rendering, "claimed-but-broken" column — partial. Per-adapter capability claims (Supported / ClaimedButBroken / Unsupported) live on `Adapter::capabilities()` since v15. The runtime side landed v51 as probe-on-first-use: `crates/atelier-core/src/adapter/model_profile.rs` writes `ModelProfile`s under `~/.atelier/model_profiles/<hash>.json` capturing the §2 strategy a model actually handles; `Event::ModelProfileLoaded` surfaces it to the GUI/TUI. Still deferred: (a) a static `capabilities.toml` lookup table consulted *before* construction for known models (alternative path to probing — useful when offline); (b) cross-walk between probe observations and `CapabilityClaim::ClaimedButBroken` so the matrix's "claimed-but-broken" column is auto-populated by the probe; (c) UI rendering of the matrix.
