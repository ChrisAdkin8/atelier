...
- [~] **Non-destructive compaction; expansion gated with cost disclosure** — compact-only landed v60.5; expand follow-up (v60.6) deferred. v60.5 ships: `SessionDispatcher::compact_context_items` + `snapshot_context_items` (atelier-core); `atelier_cli::compaction::compact` orchestrator + `atelier_cli::compaction_blob` hardened writer/reader; `LedgerEntry::Compaction` ledger variant + matching schema bump in `schemas/session/v1.json`; `Event::CompactionExecuted` bus event; `MemoryCard.compacted_from: Option<CompactionSource>` (+ `MemoryCardSummary.compacted_from: Option<u32>` projection) so v60.6 Expand finds the blob without schema migration; `ContextManager::evict_batch` atomic Pass-1 pin-check + Pass-2 evict; new `atelier_cli::AdapterHandle` mirrors `DispatcherHandle` for the new `compact_context_items` Tauri command + the TUI's `submit_compact` helper. GUI: checkbox column in `ContextPane.svelte` + "Compact N selected" header button + inline confirm dialog (`frees ~Nk tokens; reversible in v60.6`). TUI: `space` toggles multi-select + `[*]/[ ]/[-]` leftmost glyph + Shift-`C` opens `CompactConfirm` modal in `render_help` footer. Integration test in `atelier-cli/tests/run_integration.rs` (`v60_5_compact_context_items_round_trips_through_dispatcher`) asserts the full event sequence + on-disk blob round-trip. 44 new tests; workspace total 711 → 755.
