...
- **§14 concurrent-edit modal.** A file watcher tracks the dispatcher's read-set. If you (or another process) modify a file the agent has read, the next tool call pauses and the harness asks you whether to Reload (drop the queued call and re-read), Wait (keep the call queued), or Pause (auto-Reload after a timeout). The `--non-interactive` flag auto-resolves to Reload. The timeout defaults to 300 seconds and is configurable via `pause_timeout_secs` in the `[runner]` section of `providers.toml`.
...
- **Executor routing failure is fatal.** When `[routing].executor` is configured and the profile fails to load (bad credentials, unreachable endpoint, missing profile name), Atelier exits immediately with exit code 1 and an actionable error message. Remove `[routing].executor` from `providers.toml` to run without per-task routing. Both the CLI and GUI enforce this — silently falling back to the planner adapter would route tool-result turns through the wrong model without any indication.
