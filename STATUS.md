...
- **R1b: TurnState/TurnContext redesign (v60.93)** — `Runner::run` CC 140→38; `run_turn` CC 48. All `runner.rs` functions now < 50 cyclomatic. Added `runner/turn.rs` with `TurnState` (14 loop-carried vars), `TurnContext<'a>` (11 read-only refs), `TurnControl`; extracted 6 cohesive sub-methods. No behaviour change: 1,408 tests pass, clippy (workspace) clean, make check 180/180.
- **GUI footer 3-column grid (v60.94)** — `.help` footer replaced flex layout with `grid-template-columns: minmax(0, 1fr) auto minmax(0, 1fr)` so the centre token meter is always mathematically centred regardless of right-column width; `min-width: 0` on side zones lets flex content shrink without forcing tracks wider.
- **Header "thinking" sweep animation (v60.95)** — wordmark split into per-letter spans; `letter-cycle` keyframe sweeps cyan highlight left→right across "atelier" while a run is in flight (staggered `animation-delay: calc(var(--i) * 0.2s)`). Respects `prefers-reduced-motion`; `App.svelte` passes `thinking={app.runInFlight}`.
- **GUI footer model badge overlap fix (v60.96)** — grid tracks changed from `minmax(0, 1fr) auto minmax(0, 1fr)` to `auto 1fr auto`; the earlier equal-`1fr` side tracks let the right zone's `.swap-select` + `.model-badge` overflow leftward onto the token meter when a long provider label was selected. Side tracks now size to content, preventing overlap.
- **Composer readline-style prompt history (v60.97)** — `↑`/`↓` walks the localStorage-persisted history (`atelier.composer.history.v1`, 100-entry cap, bash-style dedup). First Up saves the in-flight draft; Down past the newest entry restores it. Only fires on single-line prompts with no modifiers, so multi-line editing and the slash-skill menu's own `↑`/`↓` handling are undisturbed.
- **Provider-health scan fixes (v60.103)** — F0: Mutex deadlock in `snapshot_current_model` (clone-before-emit); F1a/b: single delivery path + identity-keyed reducer reset; F2: failure arms use shared generation counter + empty-base_url scope guard; F3: retry command allowlist; F4: recovery toast clears on unreachable flap; F5/F5b: missing behavioural test + allowlist-before-credentials reorder.
- **Provider-health UX (v60.102)** — L1: `ProbeOutcome` return type, `EndpointHealthChanged` event, health dot (green/red) on model badge, unverified `?` marker on context-window cap. L2: `ProviderFailure` event, actionable failure card in conversation with Retry + Switch-profile buttons, `⚠` overlay on unreachable profile in swap dropdown. L3: background re-probe loop (30s→120s backoff, generation guard), recovery toast. Full plan: [`tasks/plan_provider_health_ux.md`](tasks/plan_provider_health_ux.md).
- **P2 quality items (v60.100)** — tool-error chains logged before stringifying; synthetic system line on resume prefix drop (model informed of dropped tool roundtrips); executor-routing fail-fast in CLI + GUI; `[runner] pause_timeout_secs` config key (default 300, wired through to concurrent-edit resolver); HOME-missing warns for subagent-type + skills registries; prod-mode `console.warn` in `castPayload` for shape-guard drift. Q6/Q8: no-action (documented).
- **CLI failure-visibility (v60.99)** — `tracing-subscriber` installed in the CLI binary (WARN+ to stderr, respects `RUST_LOG`); max_turns exhaustion now correctly sets `final_state = AwaitingUser` and emits `AgentStalled` (exit code 6, was 0); `build_and_persist_session` returns `Option<String>` error surfaced via `RunReport::persist_error` and printed by `main.rs`. One corrected pre-existing test; two new regression tests.
- **Provider hardening (v60.98)** — `DEFAULT_CONNECT_TIMEOUT_SECS = 10` added to both `AnthropicAdapter` and `OpenAiCompatAdapter` (a dead endpoint no longer blocks requests for the full 600s). `OpenAiCompatAdapter::probe_context_window()` probes `GET /v1/models` for `max_model_len` and updates the footer meter's context-window denominator; wired into `build_swap_adapter` in `atelier-gui` with a 4s `tokio::time::timeout`. `with_cache_prompt(true)` now also sets `capabilities.prompt_cache = Supported` (fixes a false-negative model badge and −5 scorer penalty). Trust boundary `PROVIDER_BASE_URL_ALLOWLIST` gains wildcard support (single `*`, host-scoped) and a `"atelier-gpu-vllm-*.elb.amazonaws.com"` entry covering any region or LB ID. `snapshot_current_model` no longer installs a `MockAdapter` fallback on default-adapter resolution failure (the mock masked the real cause and silently misrouted the first prompt).
