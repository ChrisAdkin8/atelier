...
- **Self-contained Phase A local LLM gate (v60.86)** — the nightly Phase A workflow no longer depends on Anthropic billing/secrets. It starts a CPU-only llama.cpp OpenAI-compatible server with a tiny Qwen coder GGUF model on the GitHub-hosted runner and records `phase_a_live_local_llm`.
